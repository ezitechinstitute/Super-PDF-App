...
class ExtractPagesScreen extends StatefulWidget {
  ...
  const ExtractPagesScreen({super.key, required this.selectedFile});
...
class _ExtractPagesScreenState extends State<ExtractPagesScreen> {
  ...
  Future<void> _loadPdf() async {
    ...
    try {
      ...
      debugPrint('❌ Extract Pages load error: $e');
  ...
  Future<void> _extractPages() async {
    ...
    if (_selectedPages.isEmpty) {
      _showMessage('Please select at least one page to extract.');
      return;
    }
    ...
    try {
      ...
      //
      // We process pages in their original document order,
      // so the extracted PDF keeps the same page sequence.
      ...
      if (outputBytes.isEmpty) {
        throw Exception('Extracted PDF is empty.');
      }
      ...
      if (outputFileSize <= 0) {
        throw Exception('Extracted PDF file is empty.');
      }

      final int extractedPageCount = outputDocument.pages.count;

      debugPrint('==========================================');
      debugPrint('✅ EXTRACT PAGES COMPLETED');
      debugPrint('📄 Source: ${widget.selectedFile.name}');
      debugPrint('📑 Extracted pages: $extractedPageCount');
      ...
      try {
        final response = await ApiService.instance.createHistory(
          ...
          notes:
              'Extracted $extractedPageCount '
              'page${extractedPageCount == 1 ? '' : 's'} '
              'from ${widget.selectedFile.name}.',
        );

        debugPrint(
          '✅ Extract Pages history API response: '
          '${response.data}',
        );
      } catch (historyError) {
        // PDF operation remains successful even if
        // history API temporarily fails.
        debugPrint(
          '⚠️ PDF pages extracted successfully, '
      ...
      debugPrint('❌ Extract Pages error: $e');
      ...
      _showMessage('Unable to extract the selected pages.');
    } finally {
      outputDocument?.dispose();
    }
  }
        ...
        children: [
          Expanded(
            child: _SmallActionButton(
              ...
              title: 'Select All',
              onTap: _isExtracting ? () {} : _selectAllPages,
            ),
          ),
          ...
          Expanded(
            child: _SmallActionButton(
              ...
              title: 'Clear',
              onTap: _isExtracting ? () {} : _clearSelection,
            ),
          ),
        ],
      ),
    ...
    return SliverGrid(
      delegate: SliverChildBuilderDelegate((context, index) {
        ...
        return _PageCard(
          pageNumber: index + 1,
          selected: selected,
          onTap: _isExtracting ? () {} : () => _togglePage(index),
        );
    ...
    return Padding(
      ...
      child: SizedBox(
        ...
        child: ElevatedButton.icon(
          onPressed: _isExtracting ? null : _extractPages,
          icon: _isExtracting
              ? const SizedBox(
                  width: 21,
                  height: 21,
                ...
                Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(
                        selected ? 'Selected to extract' : 'Tap to select',
                        style: TextStyle(
                          color: selected
                              ? const Color(0xFF00A8A8)
...
class _ExtractPagesBackground extends StatelessWidget {
  const _ExtractPagesBackground();
