...
class PdfToJpgScreen extends StatefulWidget {
  ...
  const PdfToJpgScreen({super.key, required this.selectedFile});
  ...
  Future<int> _calculateTotalOutputSize(List<String> generatedFiles) async {
    ...
    for (final String path in generatedFiles) {
      try {
        ...
        debugPrint('⚠️ Unable to calculate JPG size for $path: $e');
      }
    }
  ...
  Future<void> _convertToJpg() async {
    ...
    try {
      ...
      for (final int pageIndex in pagesToConvert) {
        ...
        try {
          // pdfx page number starts from 1.
          ...
          if (outputSize <= 0) {
            throw Exception(
              'Generated JPG is empty for page ${pageIndex + 1}.',
            );
          }
      ...
      if (generatedFiles.isEmpty) {
        _showMessage('No JPG images were generated.');
        return;
      }
      ...
      try {
        final String outputFileName = generatedFiles.length == 1
            ...
            : '${generatedFiles.length}_JPG_images';
        ...
        final response = await ApiService.instance.createHistory(
          ...
          notes:
              ...
              '${generatedFiles.length == 1 ? '' : 's'} to JPG.',
        );

        debugPrint(
          '✅ PDF to JPG history API response: '
          '${response.data}',
        );
      } catch (historyError) {
        debugPrint(
          '⚠️ PDF to JPG completed, but history '
      ...
      if (generatedFiles.length == 1) {
        ...
        _showMessage('JPG generated successfully.');

        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) =>
                _JpgPreviewScreen(fileName: fileName, filePath: outputPath),
          ),
        );
      ...
      _showMessage(
        '${generatedFiles.length} JPG images generated successfully.',
      );

      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => _JpgFilesScreen(filePaths: generatedFiles),
        ),
      );
    } catch (e, stackTrace) {
      debugPrint('❌ PDF to JPG conversion error: $e');
      ...
      _showMessage('Unable to convert PDF pages to JPG.');
    }
  }
    ...
    return Padding(
      ...
      child: SizedBox(
        ...
        child: ElevatedButton.icon(
          onPressed: _isConverting ? null : _convertToJpg,
          icon: _isConverting
              ? const SizedBox(
                  width: 21,
                  height: 21,
          ...
          label: Text(
            _isConverting
                ...
                : 'Convert ${_selectedPages.length} '
                      'Page${_selectedPages.length == 1 ? '' : 's'} to JPG',
                ...
                Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(
                        selected ? 'Selected for JPG' : 'Tap to select',
                        style: TextStyle(
                          color: selected
                              ? const Color(0xFFFF6B6B)
...
class _JpgPreviewScreen extends StatelessWidget {
  final String fileName;
  final String filePath;

  const _JpgPreviewScreen({required this.fileName, required this.filePath});
...
class _JpgFilesScreen extends StatelessWidget {
  final List<String> filePaths;

  const _JpgFilesScreen({required this.filePaths});
...
class _JpgResultCard extends StatelessWidget {
  final int pageNumber;
  final String fileName;
  final String filePath;

  const _JpgResultCard({
    required this.pageNumber,
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (_) =>
                _JpgPreviewScreen(fileName: fileName, filePath: filePath),
          ),
        );
      },
      child: ClipRRect(
...
class _PdfToJpgBackground extends StatelessWidget {
  const _PdfToJpgBackground();
